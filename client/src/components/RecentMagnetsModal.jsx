...
export default function RecentMagnetsModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#111927] border border-slate-200 dark:border-[#1E293B] rounded-2xl w-full max-w-2xl max-h-[88vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden">
        
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-[#1E293B] bg-white dark:bg-[#111927]">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                Deleted Magnet Links
                <span className="text-xs font-mono font-bold text-emerald-700 dark:text-[#00DF81] bg-emerald-50 dark:bg-[#00DF81]/10 px-2.5 py-0.5 rounded-full border border-emerald-200 dark:border-[#00DF81]/20">
                  {magnets.length}
                </span>
              </h3>
              <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hidden sm:inline-flex items-center gap-1">
                <Calendar className="w-3 h-3 text-emerald-500 dark:text-emerald-400" />
                Past 30 Days
              </span>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Deleted links preserved for 30 days to copy or restore</p>
          </div>
            ...
            {magnets.length > 0 && onClearAll && (
              !showClearConfirm ? (
                <button
                  ...
                  className="text-xs text-slate-500 hover:text-red-500 dark:text-slate-400 dark:hover:text-red-400 px-2.5 py-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors"
                >
                  Clear History
                </button>
              ) : (
                <div className="flex items-center gap-1 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800/60 px-2 py-1 rounded-lg text-xs">
                  <span className="text-red-600 dark:text-red-300 text-[11px]">Clear all?</span>
                  <button
                    onClick={() => {
                      onClearAll();
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3">
          {magnets.length === 0 ? (
            ...
            magnets.map((m) => {
              ...
              return (
                <div key={m.id} className="bg-slate-50 dark:bg-[#090F1C] border border-slate-200 dark:border-[#1E293B] rounded-xl p-3.5 space-y-3 relative group transition-colors hover:border-slate-300 dark:hover:border-slate-700">
                  {/* Top Row: Icon, Title & Delete */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-2.5 min-w-0 flex-1">
                      <div className="p-2 bg-slate-100 dark:bg-[#151F32] rounded-lg border border-slate-200 dark:border-[#1E293B] shrink-0 mt-0.5">
                        {getFileIcon(displayName)}
                      </div>

                      <div className="min-w-0 flex-1">
                        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100 break-words line-clamp-2" title={displayName}>
                          {displayName}
                        </h4>

                        <div className="flex flex-wrap items-center gap-1.5 mt-1">
                          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-amber-700 dark:text-amber-300 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20">
                    ...
                    {onRemove && (
                      <button
                        ...
                        className="text-slate-400 hover:text-red-500 dark:text-slate-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 p-1.5 rounded-lg transition-colors shrink-0"
