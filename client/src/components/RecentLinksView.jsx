...
export default function RecentLinksView({ 
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-[#1E293B]">
        <div>
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
              ...
              <span className="text-xs font-mono font-bold text-emerald-700 dark:text-[#00DF81] bg-emerald-500/15 dark:bg-[#00DF81]/10 px-2.5 py-0.5 rounded-full border border-emerald-500/25 dark:border-[#00DF81]/20">
                {magnets.length}
              </span>
            </h2>
            <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 flex items-center gap-1.5">
          ...
          <div>
            {!showClearConfirm ? (
              <button
                ...
                className="text-xs text-slate-500 hover:text-red-500 dark:text-slate-400 dark:hover:text-red-400 px-3.5 py-2 rounded-xl hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors border border-transparent hover:border-red-200 dark:hover:border-red-500/20 font-medium flex items-center gap-1.5"
              >
                <Trash2 className="w-3.5 h-3.5" />
                Clear 30-Day History
              </button>
            ) : (
              <div className="flex items-center gap-2 bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-800/80 px-3 py-1.5 rounded-xl text-xs shadow-lg">
                <span className="text-red-600 dark:text-red-200 font-medium">Clear all 30-day deleted links?</span>
                <button
                  onClick={() => {
                    onClearAll();
                ...
                <button
                  ...
                  className="text-slate-500 dark:text-slate-400 px-1.5 hover:text-slate-800 dark:hover:text-white text-xs"
                >
                  Cancel
                </button>
              </div>
      ...
      {magnets.length > 0 && (
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white dark:bg-[#111927] border border-slate-200 dark:border-[#1E293B] rounded-xl pl-10 pr-4 py-2.5 text-xs sm:text-sm text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-[#00DF81] transition-all"
          />
          {searchFilter && (
            <button 
              onClick={() => setSearchFilter('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800"
            >
              Clear
            </button>
          )}
        ...
        <div className="space-y-3.5">
          {filteredMagnets.map((m) => {
            ...
            return (
              <div 
                key={m.id} 
                className="bg-white dark:bg-[#111927] border border-slate-200 dark:border-[#1E293B] rounded-2xl p-4 sm:p-5 space-y-3.5 relative group shadow-sm dark:shadow-lg dark:shadow-black/20 hover:border-slate-300 dark:hover:border-slate-700 transition-all"
              >
                {/* Top Row: Icon, Title & Delete */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3.5 min-w-0 flex-1">
                    <div className="p-2.5 bg-slate-100 dark:bg-[#151F32] rounded-xl border border-slate-200 dark:border-[#1E293B] shrink-0 mt-0.5">
                      {getFileIcon(displayName)}
                    </div>

                    <div className="min-w-0 flex-1">
                      {/* Torrent File Name */}
                      <h4 className="text-sm sm:text-base font-bold text-slate-900 dark:text-slate-100 break-words line-clamp-2" title={displayName}>
                        {displayName}
                      </h4>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-1.5">
                        ...
                        <span 
                          className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-700 dark:text-amber-300 bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-500/20"
                        ...
                        {m.size && (
                          <span className="text-[11px] text-slate-700 dark:text-slate-300 font-mono bg-slate-100 dark:bg-slate-800/80 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700">
                            {formatBytes(m.size)}
                          </span>
                        )}

                        {m.deletedReason && (
                          <span className="text-[11px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-[#090F1C] px-2 py-0.5 rounded-md border border-slate-200 dark:border-[#1E293B] truncate max-w-xs">
                            {m.deletedReason}
                          </span>
                        )}
                      </div>
                  ...
                  {onRemove && (
                    <button
                      ...
                      className="text-slate-400 hover:text-red-500 dark:text-slate-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 p-2 rounded-xl transition-colors shrink-0"
